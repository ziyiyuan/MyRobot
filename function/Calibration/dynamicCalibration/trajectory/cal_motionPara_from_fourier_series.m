function motionPara = cal_motionPara_from_fourier_series(Robot, Traj, motionParaCoeff, sampleRate)
...
for i = 1 : CN
    ...
    for k = 1 : OrderNumber
        ...
        for j = 1:ARM_DOF
            sumq(j) = sumq(j) + motionParaCoeff(5 * j + k + 1) * sk + motionParaCoeff(5 * j + k + 31) * ck;
            sumqd(j) = sumqd(j) + motionParaCoeff(5 * j + k + 1) * k * waveFrequency * ck - motionParaCoeff(5 * j + k + 31) * k * waveFrequency * sk;
            sumqdd(j) = sumqdd(j) - motionParaCoeff(5 * j + k + 1) * k^2 * waveFrequency^2 * sk - motionParaCoeff(5 * j + k + 31) * k^2 * waveFrequency^2 * ck;
        end
    end
    
    for j = 1:ARM_DOF
        qt(j,i) = motionParaCoeff(j) + sumq(j);
        qtd(j,i) = sumqd(j);
        qtdd(j,i) = sumqdd(j);
    end
